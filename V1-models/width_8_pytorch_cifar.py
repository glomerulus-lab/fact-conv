...
def save_model(args, conv_model, fact_model, conv_scheduler, fact_scheduler,
        conv_optimizer, fact_optimizer):
    src= "/home/mila/v/vivian.white/scratch/v1-models/saved-models/width_8/"
    ...
    torch.save(conv_scheduler.state_dict(), trial_dir+"/conv_scheduler.pt")
    torch.save(fact_scheduler.state_dict(), trial_dir+"/fact_scheduler.pt")
    torch.save(conv_optimizer.state_dict(), trial_dir+"/conv_optimizer.pt")
    torch.save(fact_optimizer.state_dict(), trial_dir+"/fact_optimizer.pt")
...
parser.add_argument('--name', type=str, default='width8', 
...
save_model(args, conv_net, fact_net, conv_scheduler, fact_scheduler,
        conv_optimizer, fact_optimizer) 
args.name = old_name
set_seeds(0)
#if device == 'cuda':
...
for epoch in range(start_epoch, start_epoch+200):#00
    ...
    run.log(logger)
    save_model(args, conv_net, fact_net, conv_scheduler, fact_scheduler,
            conv_optimizer, fact_optimizer) # save every epoch
    args.name = old_name
