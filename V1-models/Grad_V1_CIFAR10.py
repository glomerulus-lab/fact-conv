...
import random
from hooks import forwards_hook, backwards_hook
...
if __name__ == '__main__':
    ...
    print("freeze_spatial:", args.freeze_spatial, "freeze_channel:",
            args.freeze_channel,"exponential location:", args.net)
    initial_lr = args.lr
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            ## simple module
            module.register_forward_hook(forwards_hook("{}".format(name)))
            if module.tri1_vec.requires_grad:
                module.tri1_vec.register_hook(backwards_hook("{} Tri1_vec Grad".format(name)))
            if module.tri2_vec.requires_grad:
                module.tri2_vec.register_hook(backwards_hook("{} Tri2_vec Grad".format(name)))
    ...
    cifar_data = datasets.CIFAR10(root=scattering_datasets.get_dataset_dir('CIFAR'), train=True,
    ...
    random_permute = prng.permutation(np.arange(0, 5000))[0:100]
