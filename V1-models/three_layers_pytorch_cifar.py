...
def save_model(args, conv_model, fact_model):
    src=\
    "/home/mila/v/vivian.white/scratch/v1-models/saved-models/three-layer-models-sequential/"
...
parser.add_argument('--name', type=str, default='TESTING_3Layer', 
...
run = wandb.init(project="random_project", config=args,
        group="pytorch_cifar_three_layers_sequential", name=run_name, dir=wandb_dir)
...
#originally trained my models for 90 epochs, unsure if i should train for 90 or 200
for epoch in range(start_epoch, start_epoch+args.num_epoch):#00
    train(epoch)
    test(epoch)
    conv_scheduler.step()
    fact_scheduler.step()
