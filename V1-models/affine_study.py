...
def save_model(args, conv_model, fact_model):
    src=\
    "/home/mila/v/vivian.white/scratch/v1-models/saved-models/affine_{}".format(args.affine)
...
parser.add_argument("--affine", type=int, default=1, choices=[1, 2, 3, 4],
                    help='which affine study to use')
...
def replace_layers_affine_no_weight(model):
    for n, module in model.named_children():
        if len(list(module.children())) > 0:
            ## compount module, go inside it
            replace_layers_affine_no_weight(module)
        else:
...
def replace_layers_affine_no_bias(model):
    for n, module in model.named_children():
        if len(list(module.children())) > 0:
            ## compount module, go inside it
            replace_layers_affine_no_bias(module)
        else:
            if isinstance(module, nn.BatchNorm2d):
                for name, param in module.named_parameters():
                    if "bias" in name:
...
def replace_layers_agnostic_no_affines(model, scale=1):
    prev_out_ch = 0
    for n, module in model.named_children():
        if len(list(module.children())) > 0:
            ## compound module, go inside it
            replace_layers_agnostic_no_affines(module,scale)
...
if args.affine == 1:
    ...
    run_name = "Width-Scaled Resnet Affine"
elif args.affine == 2:
    # RUN RESNET WITH NO AFFINES
    # add width (no affines)
    replace_layers_agnostic_no_affines(conv_net, args.width_scale)
    conv_net = conv_net.to(device)
    fact_net = copy.deepcopy(conv_net)
    # add fact layers
    replace_layers_keep_weight(fact_net)
    run_name = "Width-Scaled Resnet No Affine"
elif args.affine == 3:
    # RUN RESNET WITH AFFINE NO BIAS
    # add width (affines)
    replace_layers_agnostic(conv_net, args.width_scale)
    # remove bias 
    replace_layers_affine_no_bias(conv_net)
    conv_net = conv_net.to(device)
    fact_net = copy.deepcopy(conv_net)
    # add fact layers
    replace_layers_keep_weight(fact_net)
    # safety check for no bias
    replace_layers_affine_no_bias(fact_net)
    run_name = "Width-Scaled Resnet Affine No Bias"
elif args.affine == 4:
    # RUN RESNET WITH AFFINE NO WEIGHT
    # add width (affines)
    replace_layers_agnostic(conv_net, args.width_scale)
    # remove weights
    replace_layers_affine_no_weight(conv_net)
    conv_net = conv_net.to(device)
    fact_net = copy.deepcopy(conv_net)
    # add fact layers
    replace_layers_keep_weight(fact_net)
    # safety check for no weights
    replace_layers_affine_no_weight(conv_net)
    run_name = "Width-Scaled Resnet Affine No Weight"
...
run = wandb.init(project="random_project", config=args,
        group="pytorch_cifar_resnet_affine_study", name=run_name, dir=wandb_dir)
