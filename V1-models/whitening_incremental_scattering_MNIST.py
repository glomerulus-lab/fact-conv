...
if __name__ == '__main__':
    ...
    filename = "generative_scattering_results/mnist/scattering/"+args.filename
    ...
    whitened_batch_train = torch.from_numpy(whitener.transform(scattering_fixed_batch.view(scattering_fixed_batch.size(0), -1).cpu().detach().numpy())).float().to(device)
    ...
    whitened_batch_test = torch.from_numpy(whitener.transform(scattering_fixed_batch_test.view(scattering_fixed_batch_test.size(0), -1).cpu().detach().numpy())).float().to(device)
