...
def save_model(args, conv_model, fact_model):
    src=\
    "/home/mila/v/vivian.white/scratch/v1-models/saved-models/comparing_k_new/"
...
parser.add_argument('--k', type=int, default=1, 
                    help='k value for FactConv2dK model')
...
#
#
from ConvModules import FactConv2dPreExp, FactConv2dK

def replace_layers_keep_weight_K(model):
    for n, module in model.named_children():
        if len(list(module.children())) > 0:
            ## compound module, go inside it
            replace_layers_keep_weight_K(module)
        if isinstance(module, nn.Conv2d):
            ## simple module
            new_module = FactConv2dK(
...
if args.net == "vgg":
    ...
    net = ResNet18()
    conv_net = ResNet18()
    replace_layers_keep_weight(conv_net)
    run_name = "test_K={}".format(args.k)
# REMINDER THAT CONV = OG FACT AND FACT = K FACT
conv_net = conv_net.to(device)
fact_net = copy.deepcopy(conv_net)
replace_layers_keep_weight_K(fact_net)
...
run = wandb.init(project="random_project", config=args,
        group="pytorch_cifar_k_comparisons", name=run_name, dir=wandb_dir)
