#runs from script1 and script2 from generate_params.py
...
class BN_V1_V1_LinearLayer(nn.Module):
    ...
    def forward(self, x):  #[128, 3, 32, 32]
        ...
        layer1 = self.v1_layer(x)  #[128, hidden_dim, 32, 32] w/ k=7, s=1, p=3
        layer1bn = self.bn1(layer1)
        
        layer2 = self.v1_layer2(layer1bn) #COULD ADD BATCH NORM HERE #[128, hidden_dim, 32, 32] w/ k=7, s=1, p=3
        layer2bn = self.bn2(layer2)
        
        x1 = self.relu(x)
        h1 = self.relu(layer1bn)
        h2 = self.relu(layer2bn)
        
        pool = nn.AvgPool2d(kernel_size=4, stride=4, padding=1)  
        x_pool = pool(x1) #[128, 3, 8, 8]
        h1_pool = pool(h1)  #[128, hidden_dim, 8, 8]
        h2_pool = pool(h2)  #[128, hidden_dim, 8, 8]
...
def test(model, device, test_loader, epoch):
    ...
    if epoch == 90:
        print('Loss: {:.4f}\t Accuracy: {:.2f}%'.format(test_loss, accuracy))

    return test_loss, accuracy

def save_model(args, model):
    src = "/research/harris/vivian/v1-models/saved-models/BN_V1_V1_Linear/"
    model_dir =  src + "s_f_sweep/" + args.name
...
if __name__ == '__main__':
    ...
    parser.add_argument('--s', type=int, default=3, help='V1 size')
    parser.add_argument('--f', type=float, default=1.0, help='V1 spatial frequency')
    ...
    model = BN_V1_V1_LinearLayer(args.hidden_dim, args.s, args.f, args.scale, args.bias).to(device)
