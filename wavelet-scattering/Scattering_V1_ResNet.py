...
class ScatterV1ResNet(nn.Module):
    def __init__(self, in_channels,  hidden_dim, size, spatial_freq, center=None, scale=1, bias=False, seed=None, k=2, n=4, num_classes=10):
        super(ScatterV1ResNet, self).__init__()
        self.inplanes = 16 * k
        self.ichannels = 16 * k
        self.K = in_channels
        ...
        self.v1_layer = nn.Conv2d(51, out_channels=hidden_dim, kernel_size=8, stride=1) 
        self.relu = nn.ReLU()
    ...
    def forward(self, x): #[128, 3, 17, 8, 8]
        x = x.view(x.size(0), self.K, 8, 8) #[128, 51, 8, 8]=
        v1 = self.bn(self.v1_layer(x)) #[128, hidden_dim, 1, 1]
        #v1 = self.v1_layer(x)
        h = self.relu(v1) 
        x = self.init_conv(h) 
        x = self.layer2(x)
...
def test(model, device, test_loader, scattering, epoch):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    scale = 1
    model = ScatterV1ResNet(K, h, s, f, scale=scale, center=None).to(device)
    ...
    for epoch in range(0, 90):
        ...
        test(model, device, test_loader, scattering, epoch+1)
