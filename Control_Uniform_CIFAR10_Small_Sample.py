...
def save_model(args, model, test_loss, test_accuracy):
    src = "/research/harris/vivian/v1-models/saved-models/Control_Uniform_CIFAR10_Small_Sample/"
...
def main():
    ...
    start = datetime.now()
    model = Control_Uniform(args.hidden_dim, args.s, args.f, args.scale, args.bias).to(device)
    # DataLoaders
    if use_cuda:
        num_workers = 4
        pin_memory = True
