...
class ResNet(nn.Module):
    def __init__(self,  k=2, n=4, num_classes=10):
    ...
    def forward(self, x): #[128, 3, 32, 32\
        x = self.init_conv(x) #[128, 32, 8, 8]
        x = self.layer2(x) #[128, 64, 8, 8]
        x = self.layer3(x) #[128, 128, 8, 8]
        x = self.avgpool(x) #[128, 128, 2, 2]
        x = x.view(x.size(0), -1) #[128, 512]
        x = self.fc(x) #[128,10]
        return x
...
if __name__ == '__main__':
    ...
    model = ResNet(args.width).to(device)
