    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                ...
                nn.ReLU(),
                Alignment(self.expansion*planes, self.expansion*planes),
                nn.BatchNorm2d(self.expansion*planes),
                #nn.BatchNorm2dplanes),
            )

    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.bn_align1(self.align1(out))
        out = F.relu(self.bn2(self.conv2(out)))
        out = self.bn_align2(self.align2(out))
        out += self.shortcut(x)
        out = F.relu(out)
        return out
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.bnalign = nn.BatchNorm2d(64)
        ...
        self.bn_final = nn.Identity()#nn.BatchNorm2d(512*block.expansion)
    ...
    def forward(self, x):
        ...
        #out = F.relu(self.bn1(self.conv1(x)))
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.bnalign(self.align1(out))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
