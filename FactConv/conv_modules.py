...
class FactConv2d(nn.Conv2d):
    ...
    def forward(self, input: Tensor) -> Tensor:
        U1 = self.channel._tri_vec_to_mat(self.channel.tri_vec,
                self.channel.triu_size, self.channel.scat_idx)
        U2 = self.spatial._tri_vec_to_mat(self.spatial.tri_vec,
                self.spatial.triu_size, self.spatial.scat_idx)
...
class DiagChanFactConv2d(nn.Conv2d):
    def __init__(
        ...
        #mask = triu1[0] == triu1[1]
        #scat_idx1 = triu1[0][mask]*self.in_channels//self.groups + triu1[1][mask]
        matrix = torch.arange(self.in_channels // self.groups \
                * self.in_channels // self.groups).view(self.in_channels // \
                        self.groups, self.in_channels // self.groups)
        scat_idx1 = torch.diagonal(matrix)
        self.register_buffer("scat_idx1", scat_idx1, persistent=False)
