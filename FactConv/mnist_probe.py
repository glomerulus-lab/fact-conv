...
run = wandb.init(project="FactConv", entity="whitev4", config=args,
        group="eval_mnists(test)",
        #name="{}_{}_{}_{}_{}_{}_align{}_abs".format(args.dataset,args.nonlin,
...
class Alignment(nn.Module):
    ...
    def __init__(self, size, rank):
        ...
        self.svd = SVD.apply
        self.size = size
        self.rank = rank #args.align_rank
        assert self.rank <= self.size, "Rank must be less/equal to size"
        #if size < rank+1:
    ...
    def forward(self, x):
        ...
        assert x.shape == 2 * self.size, "Self.size and x shape incompatible"
        if self.state == 1:
            return x
        # changing path
        x1 = x[:, 0 : x.shape[1]//2]#, :, :]
...
class NewAlignment(nn.Module):
    ...
    def __init__(self, size, rank, mode='average', mom=1.0):
        ...
        #else:
        #    self.size = rank
        self.cov = torch.zeros((self.size,self.size)).cuda()
        self.total = 0
        self.mode=mode
        self.mom=mom
        alignment=torch.zeros((self.size,self.size)).cuda()
...
def realign(model):
    for (n1, m1) in model.named_children():
        ...
        if isinstance(m1, Alignment):
            setattr(model, n1, NewAlignment(m1.size, m1.rank))
...
logger['minibatch_test_acc'] = pre_resample_test['acc']
logger['minibatch_test_loss'] = pre_resample_test['loss']
