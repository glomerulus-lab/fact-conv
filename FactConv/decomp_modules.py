...
class EighFactConv2d(nn.Conv2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        self.tri1_weight = nn.Parameter(torch.eye(self.in_channels //
            self.groups))
        self.tri2_weight = nn.Parameter(torch.eye(self.kernel_size[0] * self.kernel_size[1]))
    ...
    def _tri_vec_to_mat(self, vec, n, scat_idx):
        ...
        U = torch.diagonal_scatter(U, torch.sqrt(torch.abs(U.diagonal())))# .exp_()
        return U

class EighFixedFactConv2d(nn.Conv2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(self, input: Tensor) -> Tensor:
        U1 = self._tri_vec_to_mat(self.tri1_vec, self.in_channels //
                                  self.groups, self.scat_idx1)
        U1 = torch.mm(self.tri1_weight.T, torch.mm(U1, self.tri1_weight))
        U2 = self._tri_vec_to_mat(self.tri2_vec, self.kernel_size[0] * self.kernel_size[1],
                self.scat_idx2)
        #U2 = torch.mm(U2, self.tri2_weight)
        U2 = torch.mm(self.tri2_weight.T, torch.mm(U2, self.tri2_weight))
        # flatten over filter dims and contract
        composite_weight = _contract(self.weight, U1.T, 1)
        composite_weight = _contract(
            torch.flatten(composite_weight, -2, -1), U2.T, -1
...
class EighFixedDoubleFactConv2d(nn.Conv2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class EighDoubleFactConv2d(nn.Conv2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class RQFactConv2d(nn.Conv2d):
    def __init__(
        ...
        self.tri1_weight = nn.Parameter(torch.eye(self.in_channels //
            self.groups)).to(self.weight.device)
        self.tri2_weight = nn.Parameter(torch.eye(self.kernel_size[0] * self.kernel_size[1])).to(self.weight.device)
...
class RQLeftFactConv2d(nn.Conv2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(self, input: Tensor) -> Tensor:
        U1 = self._tri_vec_to_mat(self.tri1_vec, self.in_channels //
                                  self.groups, self.scat_idx1)
        U1 = torch.mm(self.tri1_weight, U1)
        U2 = self._tri_vec_to_mat(self.tri2_vec, self.kernel_size[0] * self.kernel_size[1],
                self.scat_idx2)
        U2 = torch.mm(self.tri2_weight, U2)
        # flatten over filter dims and contract
        composite_weight = _contract(self.weight, U1.T, 1)
        composite_weight = _contract(
            torch.flatten(composite_weight, -2, -1), U2.T, -1
...
class RQLeftDoubleFactConv2d(nn.Conv2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class RQDoubleFactConv2d(nn.Conv2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
