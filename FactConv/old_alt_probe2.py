...
def realign(model):
    for (n1, m1) in model.named_children():
        ...
        if isinstance(m1, AltAlignment):
            setattr(model, n1,
                    NewAltAlignment(m1.rank,m1.rank,args.mode,args.mom))
...
def load_model(args, model):
    ...
    #run_name= "{}_batchsize_{}_rank_{}_resample_{}_width_{}_seed_{}_epochs_{}_k_{}".format(args.net,
            args.batchsize, args.rank,
            #1 if args.width == 0.125 else args.double, args.resample,
            args.resample,
              args.width, args.seed, 
              args.num_epochs,
...
parser.add_argument('--mode', default='average', type=str, choices=['average','momentum'])
parser.add_argument('--mom', default=1.0, type=float)
...
run_name = "rank_{}_optimize_{}_{}_batchsize_{}_rank_{}_{}_resample_{}_width_{}_seed_{}_epochs_{}".format(
        args.channel_k, args.optimize,args.net, args.batchsize, args.rank, args.double,
...
run_name= "rank_{}_optimize_{}_statistics_{}_resampleseed_{}_{}_batchsize_{}_rank_{}_{}_resample_{}_width_{}_seed_{}_epochs_{}".format(
        args.channel_k, args.optimize,
...
run = wandb.init(project="FactConv", entity="whitev4", config=args,
        ...
        #group="seeds_loading_probing_align_resnet_cifar", name=run_name, dir=wandb_dir)
...
parameters = net.linear.parameters() if "alt_aligned" not in args.net else net.resnet.fc.parameters()
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if state == 1:
            ...
            for i in range(0, num_ensemble_samples):
                resample(net)
                if outputs is None:
                    outputs = net(inputs)/num_ensemble_samples
                else:
                    outputs += net(inputs)/num_ensemble_samples
        else: 
            outputs = net(inputs)[inputs.shape[0]:]
...
def test(epoch, loader, state=1, num_ensemble_samples=10):
    global best_acc
    net.eval()
...
if "align" in args.net:
    ...
    test(epoch=0, state=2, loader=testloader, num_ensemble_samples=10)
    ...
    for epoch in range(0, 5):
        train(epoch, 0)
        test(epoch=epoch, loader=testloader, state=0)
