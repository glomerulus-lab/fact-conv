...
run = wandb.init(project="FactConv", entity="muawizc", config=args,
        group="testing_ensemble", name=run_name, dir=wandb_dir)
...
#TODO: CHANGE
classifier = nn.Linear(10*10, 10)
classifier.cuda()
optimizer = optim.SGD(classifier.parameters(), lr=0.1, momentum=0.9,
        weight_decay=5e-4)
args.resample=0
logger = {}
#nets = [copy.deepcopy(net) for i in range(0, )]
...
#    set_seeds(i)
#    resample(net)
#    torch.save(net.state_dict(), model_dir+ "/model_{}.pt".format(i))
print("NETS COPIED")
#[resample(net_i) for net_i in nets]
net.cuda()
print("DONE SEEDING")
# Training
def train(epoch, nets):
    print('\nEpoch: %d' % epoch)
    #if args.statistics:
    #    net.train()
    #else:
...
def test(epoch, nets, net_c, data_dict):
    global best_acc
    #net.eval()
...
#with open('saved_dictionary_ensemble_trained_5_random.pkl', 'wb') as f:
with open('saved_dictionary_ensemble_rqfree_100_trained_5.pkl', 'wb') as f:
#with open('saved_dictionary_ensemble_100_trained_5.pkl', 'wb') as f:
